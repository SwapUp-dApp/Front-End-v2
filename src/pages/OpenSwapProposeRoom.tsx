...
const OpenSwapProposeRoom = () => {
  ...
  const [receiverDataSaveInStore, setReceiverDataSaveInStore] = useState(false);
  ...
  return (
    <div className="space-y-4" >
      ...
      <div className="grid lg:grid-cols-2 gap-4 pb-16" >
        ...
        {isSuccess && (state.receiver.profile.walletAddress) ?
          <RoomLayoutCard
            ...
            layoutType={"receiver"}
            roomKey="openRoom"
            setDataSavedInStore={setReceiverDataSaveInStore}
          />
          :
          <div className="rounded-sm border-none w-full h-full flex items-center justify-center dark:bg-su_secondary_bg p-2 lg:p-6" >
            {
      ...
      <footer className="bg-su_primary_bg fixed bottom-0 left-0 w-full min-h-[112px] lg:h-[104px] flex justify-between" >
        ...
        {
          receiverDataSaveInStore ?
            <RoomFooterSide showRemoveNftButton={false} roomKey="openRoom" layoutType="receiver" setEnableApproveButtonCriteria={setEnableApproveButtonCriteria} />
            :
            <div className="w-1/2 p-4 border border-su_disabled flex items-center justify-center" >
              <LoadingDataset
                isLoading={!receiverDataSaveInStore}
