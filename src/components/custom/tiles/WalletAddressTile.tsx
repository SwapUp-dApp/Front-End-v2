  ...
  return (
    <div className="flex items-center justify-between lg:justify-start xl:gap-20 lg:gap-16 dark:bg-su_enable_bg py-2 px-3 rounded-xs" >
      <div
        ...
        {...props}
      >
        <svg className="w-3 lg:w-4" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className="hidden lg:inline-block pt-[4.5px]" >{!showLongWalletAddressOnLargeScreen ? getShortenWalletAddress(walletAddress) : walletAddress}</span>
        <span className="lg:hidden pt-[4.5px]" >{!showLongWalletAddressOnMobileScreen ? getShortenWalletAddress(walletAddress) : walletAddress}</span>
      </div>
