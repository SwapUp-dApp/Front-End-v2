...
const WalletAddressTile = ({ walletAddress, className, ...props }: IProp) => {
  return (
    <div
      className={cn(
        "flex items-center gap-1 lg:gap-2 text-xs lg:text-sm dark:bg-su_enable_bg py-2 px-3 rounded-xs",
        className
      )}
      {...props}
    >
