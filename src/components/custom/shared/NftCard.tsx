...
const NftCard = ({ className, topRated = true, ...props }: IProp) => {
  ...
  return (
    <div
      className={cn(
        `${activeGridView === "detailed" ? "h-[196px]" : "h-[100px]"} bg-su_enable_bg rounded-md cursor-pointer relative group`,
        className
      )}
      {...props}
    >
      <div className={`relative ${activeGridView === "detailed" ? "h-[132px] rounded-tl-md rounded-tr-md" : "h-full rounded-md"}`}>
        ...
        <svg className="absolute top-4 right-6 w-1" viewBox="0 0 4 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={`hidden group-hover:flex absolute p-2 rounded-sm bg-white text-black flex-col gap-1 ${activeGridView === "detailed" ? "bottom-7 left-2 text-sm" : "-bottom-6 z-10 text-3xs"} font-semibold`} >
        <p>Top 0.1%</p>
        <div>
