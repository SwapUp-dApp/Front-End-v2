...
const SharingSwapCompletionOnSocialDialog = ({ open, setOpen, title, description }: IProp) => {
  ...
  const shareSwapPostOnSocialPlatform = (socialPlatform: "TWITTER" | "WARPCAST") => {

    const hashtagString = ["SwapUp"].join(",");
    const postContent: string = `Check out my recent ${swap?.swap_mode === SUE_SWAP_MODE.OPEN ? "open" : "private"} Swap: 

    ${JSON.stringify(swap?.metadata.init.tokens.map(token => token.address))}
    \nto
    ${JSON.stringify(swap?.metadata.accept.tokens.map(token => token.address))}
    
    https://white-dune-0fb9b1810.5.azurestaticapps.net/  
    \n`;
    ...
    switch (socialPlatform) {
      case 'TWITTER':
        socialMediaUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(postContent)}&hashtags=${encodeURIComponent(hashtagString)}`;
        break;
      case 'WARPCAST':
        socialMediaUrl = `https://warpcast.com/cast?text=${encodeURIComponent(postContent)}`;
        break;
    }
      ...
      <DialogContent className="w-[400px] p-4 space-y-3" >
        ...
        <div className="bg-su_least_bg p-4 rounded-md flex items-center justify-between" >
          ...
          <span className="flex items-center gap-2" >
            <CustomIconButton
              onClick={() => { shareSwapPostOnSocialPlatform('TWITTER'); }}
            >
            ...
            <CustomIconButton onClick={() => { shareSwapPostOnSocialPlatform("WARPCAST"); }} >
              <svg className="w-4" viewBox="0 0 17 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
