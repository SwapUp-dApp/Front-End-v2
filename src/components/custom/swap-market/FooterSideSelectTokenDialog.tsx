...
const FooterSideSelectTokenDialog = ({ open, setOpen, handleSetSelectedCurrency }: IProp) => {
  ...
  const handleSearchFilterNotPreferredCurrenciesList = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFilteredNotPreferredCurrencyList = notPreferredCurrenciesList.filter(currency => {
      const currencyEthereumAddress = (currency.contractAddresses as string[]).find(currency => currency.startsWith('ethereum'));

      return (
        currency.name.toLowerCase().includes(searchValue) ||
        currencyEthereumAddress?.includes(searchValue)
      );
    });
                      ...
                      <FormControl>
                        ...
                        >
                          <div className="flex items-center gap-2 flex-wrap p-1">
                            {preferredCurrenciesList.map(currency => {
                              const currentValue = JSON.stringify(currency);
                              return (
                                <FormItem key={currency.uuid} className="p-0">
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormControl>
                          ...
                          >
                            <div className="flex items-center gap-2 flex-wrap p-1">
                              {filteredNotPreferredCurrenciesList.map(currency => {
                                const currentValue = JSON.stringify(currency);
                                return (
                                  <FormItem
                                    key={currency.uuid}
                                  ...
                                  >
                                    ...
                                    >
                                      <img src={currency.iconUrl} alt="" className="w-6 h-6 mt-1" />
